...
class LaporanProgram extends FPDF
{
    ...
    public function setContent(array $data, string $jenisPembayaran)
    {
        $this->Ln(20);
        ...
        $this->Cell(0, 0, 'Data Program ' . $this->jenis_program . " $jenisPembayaran", 0, 1, 'C');
        $this->Ln(10);
    ...
    public function content(array $data)
    {
        ...
        if (!is_null($jenisUang)) {
            $data = $this->model->getLaporan($this->jenis_program, $jenisUang);
            $this->setContent($data, 'Uang');
        }

        if (!is_null($jenisBarang)) {
            $data = $this->model->getLaporan($this->jenis_program, $jenisBarang);
